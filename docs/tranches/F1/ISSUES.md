...
- **Status:** ALL RESOLVED_AT_SPEC_FREEZE (10/10 specification issues resolved under Product Owner freeze; runtime verification pending BUILD stage)
- **Current Lifecycle Gate:** `SPEC_FROZEN` (Specification resolved under explicit Product Owner Freeze; Runtime Evidence Gate: `BUILD_VALIDATION`)
- **Issues Blocking BUILD Entry:** 0 blocking Work Order authoring (10 require empirical runtime verification during BUILD stage)
- **Disposition Breakdown:**
  - `RESOLVED_AT_SPEC_FREEZE`: 10 (`ISSUE-F1-001` through `ISSUE-F1-010`)
- **Target Resolution Gates:**
  - `SPEC_FREEZE`: 10 (RESOLVED under `SPEC-FOUNDATION-F1-001` v1.0.0; Product Owner Freeze Decision: APPROVED; Independent Review: `REV-F1-SPEC-001-R5` PASS / `PASS_FOR_FREEZE`)
...
|---|---|---|---|:---:|:---:|:---:|:---:|
| `OQ-F1-01` | Drawing-unit enforcement vs validation/warning | `INTAKE.md` §12 | **ISSUE-F1-001** | NO (Resolved at Spec Freeze) | `SPEC_FREEZE` | **RESOLVED_AT_SPEC_FREEZE** | `BUILD_VALIDATION` |
| `OQ-F1-02` | Handling non-millimeter, unitless, and conflicted DWGs | `INTAKE.md` §12 | **ISSUE-F1-002** | NO (Resolved at Spec Freeze) | `SPEC_FREEZE` | **RESOLVED_AT_SPEC_FREEZE** | `BUILD_VALIDATION` |
| `OQ-F1-03` | Geometric tolerance model: single $\varepsilon$ vs typed tolerances | `INTAKE.md` §12 | **ISSUE-F1-003** | NO (Resolved at Spec Freeze) | `SPEC_FREEZE` | **RESOLVED_AT_SPEC_FREEZE** | `BUILD_VALIDATION` |
| `OQ-F1-04` | `TTC_OBJECT_ID` generation format & uniqueness scope | `INTAKE.md` §12 | **ISSUE-F1-004** | NO (Resolved at Spec Freeze) | `SPEC_FREEZE` | **RESOLVED_AT_SPEC_FREEZE** | `BUILD_VALIDATION` |
| `OQ-F1-05` | Identity lifecycle under native clone/copy operations | `INTAKE.md` §12 | **ISSUE-F1-005** | NO (Resolved at Spec Freeze) | `SPEC_FREEZE` | **RESOLVED_AT_SPEC_FREEZE** | `BUILD_VALIDATION` |
| `OQ-F1-06` | Metadata storage split: XData vs Extension Dictionary/XRecord | `INTAKE.md` §12 | **ISSUE-F1-006** | NO (Resolved at Spec Freeze) | `SPEC_FREEZE` | **RESOLVED_AT_SPEC_FREEZE** | `BUILD_VALIDATION` |
| `OQ-F1-07` | Schema versioning, migration, and backward compatibility | `INTAKE.md` §12 | **ISSUE-F1-007** | NO (Resolved at Spec Freeze) | `SPEC_FREEZE` | **RESOLVED_AT_SPEC_FREEZE** | `BUILD_VALIDATION` |
| `OQ-F1-08` | Native AutoCAD event/reactor strategy vs command-boundary audit | `INTAKE.md` §12 | **ISSUE-F1-008** | NO (Resolved at Spec Freeze) | `SPEC_FREEZE` | **RESOLVED_AT_SPEC_FREEZE** | `BUILD_VALIDATION` |
| `OQ-F1-09` | Mechanical block asset contracts, scaling, and orientation rules | `INTAKE.md` §12 | **ISSUE-F1-009** | NO (Resolved at Spec Freeze) | `SPEC_FREEZE` | **RESOLVED_AT_SPEC_FREEZE** | `BUILD_VALIDATION` |
| `OQ-F1-010` | Corrupt, orphan, or missing metadata recovery strategy | `INTAKE.md` §12 | **ISSUE-F1-010** | NO (Resolved at Spec Freeze) | `SPEC_FREEZE` | **RESOLVED_AT_SPEC_FREEZE** | `BUILD_VALIDATION` |
...
- **Status:** RESOLVED_AT_SPEC_FREEZE
- **Specification Resolution:** RESOLVED
- **Resolution Authority:** `PRODUCT_OWNER_F1_SPEC_FREEZE`
- **Frozen Spec:** `SPEC-FOUNDATION-F1-001` v1.0.0
- **Severity:** HIGH
- **Category:** DOMAIN / UNITS
- **Owner:** Product Owner / Architect
- **Blocks Entry To BUILD:** NO (Specification resolved at SPEC_FREEZE)
- **Required Closure Gate:** `SPEC_FREEZE` (SATISFIED)
- **Verification Evidence Gate:** `BUILD_VALIDATION`
- **Runtime Evidence:** PENDING `BUILD_VALIDATION`
...
- **SPEC Formalization (`SPEC-FOUNDATION-F1-001` v1.0.0 §3.2, §3.3, §3.5, `AC-F1-02`, `AC-F1-03`, `AC-F1-04`, `TEST-F1-16`, `TEST-F1-17`, `TEST-F1-28`):**
...
- **SPEC Formalization (`SPEC-FOUNDATION-F1-001` v1.0.0 §3.2, §3.4, §15.1, `AC-F1-02`, `AC-F1-03`, `TEST-F1-16`, `TEST-F1-17`):**
...
- **SPEC Formalization (`SPEC-FOUNDATION-F1-001` v1.0.0 §3.3, §4, `AC-F1-04`, `AC-F1-05`, `TEST-F1-28`):**
...
- **SPEC Formalization (`SPEC-FOUNDATION-F1-001` v1.0.0 §5, §11, `AC-F1-06`, `AC-F1-12`, `TEST-F1-12`):**
...
- **SPEC Formalization (`SPEC-FOUNDATION-F1-001` v1.0.0 §10, §10.1, §11, §12.2, `AC-F1-07`, `AC-F1-09`, `AC-F1-10`, `AC-F1-11`, `AC-F1-12`, `AC-F1-18`, `TEST-F1-02`, `TEST-F1-03`, `TEST-F1-10`, `TEST-F1-11`, `TEST-F1-12`, `TEST-F1-15`, `TEST-F1-21`):**
  - Comprehensive native edit lifecycle matrix formalized (§10, §10.1), categorizing commands into:
    ...
    - **Category B (Cross-Database Export / Out-of-Scope Commands):** `WBLOCK` (exports objects to a secondary database on disk; target DWG considered a valid TTC artifact MUST persist distinct `TTC_OBJECT_ID` values; target is not promoted or accepted as a valid project drawing while containing duplicate IDs; exact host clone mechanism remains `BUILD_VALIDATION_REQUIRED / HOST_TEST_REQUIRED` in `TEST-F1-12`, addressing R4-F02).
...
- **SPEC Formalization (`SPEC-FOUNDATION-F1-001` v1.0.0 §7, §8, §8.4, `AC-F1-13`, `AC-F1-14`, `AC-F1-15`, `AC-F1-16`, `TEST-F1-04`, `TEST-F1-27`):**
...
- **SPEC Formalization (`SPEC-FOUNDATION-F1-001` v1.0.0 §7.3, §7.4, §9, §9.2, §9.3, §15.2, `AC-F1-17`, `TEST-F1-13`, `TEST-F1-20`, `TEST-F1-29`):**
...
- **SPEC Formalization (`SPEC-FOUNDATION-F1-001` v1.0.0 §12, §12.2, §12.4, §13, `AC-F1-21`, `TEST-F1-09`, `TEST-F1-15`, `TEST-F1-23`, `TEST-F1-26`):**
...
- **SPEC Formalization (`SPEC-FOUNDATION-F1-001` v1.0.0 §14, §15.1, `AC-F1-04`, `AC-F1-22`, `TEST-F1-24`, `TEST-F1-28`):**
...
- **SPEC Formalization (`SPEC-FOUNDATION-F1-001` v1.0.0 §8.4, §11.2, §15.1, §15.2, `AC-F1-15`, `AC-F1-16`, `TEST-F1-19`, `TEST-F1-27`):**
  - Standardized deterministic failure statuses: strictly 10 persistent entity metadata failure classifications in §15.1, cleanly separated from transient operation execution results in §15.2 (`OperationExecutionResult != EntityFailureClassification`, addressing R4-F01).
