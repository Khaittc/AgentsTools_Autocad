...
- **Host Mechanism:** Native command behaviors cataloged in SRC-F1-09 (*AutoCAD Commands That Use Deep Clone and Wblock Clone*):
  - `COPY`: invokes `deepClone`.
  - `ARRAY`: invokes `deepClone`.
  - `MIRROR`:
    - If original and mirrored objects are both preserved: `deepClone` is used, creating a new entity with a distinct Handle.
    - If original objects are erased: `deepClone` is NOT used; the original object is mirrored in place without minting a new Handle.
- **Documented Semantics (SRC-F1-01, SRC-F1-02, SRC-F1-03, SRC-F1-09):**
  - **AutoCAD Handle:** When `deepClone` executes (`COPY`, `ARRAY`, or preserving `MIRROR`), the cloned entity is assigned a brand new, unique AutoCAD `Handle` by the target `Database`.
...
- **Pre-Save Mutation Safety (`Database.BeginSave`):**
  - Mutating database entities or regenerating XRecords inside the `Database.BeginSave` event handler is **unverified** and carries high risk of database corruption, recursive save triggers, or file save lock conflicts.
  - **Classification:** `HOST_TEST_REQUIRED`. `BeginSave` must be verified via automated host test before SPEC may rely on it for database writes. Passive duplicate detection/logging is preferred. Any user interaction must be deferred to a safe execution context outside the event callback.
- **Verification Status:**
  - `Handle` uniqueness on clone: `HOST_FACT_SOURCE_VERIFIED` (SRC-F1-02).
  - Selective `MIRROR` deep-clone behavior: `HOST_FACT_SOURCE_VERIFIED` (SRC-F1-09).
  - Extension dictionary duplication: `HOST_FACT_SOURCE_VERIFIED` by documentation (SRC-F1-03, SRC-F1-09); runtime behavior across varied entity types is `HOST_TEST_REQUIRED` for BUILD verification.

---

### API-F1-02: Drawing Unit Variable Semantics & Conflict Detection (`INSUNITS`, `MEASUREMENT`, `LUNITS`)

- **Topic:** Host drawing-level unit flags, fallback variables, and their interplay with physical engineering units and project configurations.
...
- **Architectural Consequence for F1:**
  ...
  - **Configuration Conflict Resolution:** When an explicit project/workspace unit configuration (e.g. `ProjectConfig = Millimeters`) contradicts an explicit non-zero host `INSUNITS` setting (e.g. `INSUNITS = 1` - Inches), the system enters a distinct **`UNIT_CONFIGURATION_CONFLICT`** state. Neither source is silently favored. Execution of physical insertion and automated scaling is blocked until the conflict is resolved by approved policy or user confirmation.
...
- **Documented Semantics (SRC-F1-03, SRC-F1-04):**
  - **Extended Data (`DBObject.XData`):**
    ...
    - Uses DXF group codes in the 1000–1071 range (e.g., `1000` for ASCII string, `1001` for RegAppName, `1071` for 32-bit integer).
  ...
  - **Extension Dictionary (`DBObject.ExtensionDictionary`) & `XRecord`:**
    ...
    - Encodes data in `ResultBuffer` chains using standard DXF group codes < 1000 (specifically group code `1` / `DxfCode.Text` for string keys and values; group codes >= 1000 are reserved strictly for XData).
    - Storage capacity bounded by the **2 GB per XRecord limit** supported by the AutoCAD database architecture (`AcDbXrecord::kMaxDataSize` / database object limits), substantially larger than XData. However, TTC metadata records must remain compact (< a few KB). XRecord is NOT described as unlimited.
...
- **Documented Host Facts (SRC-F1-05):**
  ...
  - Note: The AutoCAD database does not expose a database transaction sequence counter or entity modification timestamp API. Tracking changes by "comparing transaction sequence or timestamp" is unsupported by native host APIs.
- **TTC Architectural Policy (`PROJECT_POLICY`):**
  ...
  - Reactors may only set in-memory `DIRTY` flags or invalidate ephemeral in-memory caches.
  - Deep validation, identity auditing, and duplicate reconciliation execute strictly at **command boundaries** via `Document.CommandEnded`, `Document.CommandCancelled`, and `Document.CommandFailed` events on `Autodesk.AutoCAD.ApplicationServices.Document` (or during explicit QA audit commands). Note: `Editor` does not expose a `CommandEnded` event in AutoCAD Managed .NET.
  - Command event handlers remain observation-only; any subsequent reconciliation writes require an explicit `DocumentLock` and database `Transaction`.
  - Derived layout models and spatial indexes are reconstructed on demand from persistent DWG geometry and TTC metadata.
  - Active document activation/switching immediately invalidates ephemeral caches.
- **Verification Status:**
...
- **Documented Semantics (SRC-F1-01, SRC-F1-03, SRC-F1-09):**
  - Per Autodesk Developer Guide (*AutoCAD Commands That Use Deep Clone and Wblock Clone*):
    - `WBLOCK`: uses `wblockClone` to export selected entities into a newly generated DWG database. Extension dictionaries attached to entities are cloned into the target file.
    - `COPYCLIP`: uses `wblockClone` to write selected entities into a temporary clipboard DWG database.
    - `PASTECLIP`: uses `wblockClone` to transfer entities from the clipboard DWG into the active drawing database, assigning new Handles.
    - `INSERT`: uses `deepClone` when inserting external drawings or blocks into the current drawing.
...
- **Verification Status:**
  - Host cloning command classifications: `HOST_FACT_SOURCE_VERIFIED` (SRC-F1-09).
  - Runtime verification of TTC metadata during inter-drawing transfer: `HOST_TEST_REQUIRED` (Automated verification suite required during BUILD stage).
...
| XRecord up to 2 GB database limit | `HOST_FACT_SOURCE_VERIFIED` | Supported by AutoCAD database architecture; TTC metadata kept compact. |
| Command clone mechanics (COPY, MIRROR, WBLOCK, etc.) | `HOST_FACT_SOURCE_VERIFIED` | Cataloged in Autodesk DevGuide (*AutoCAD Commands That Use Deep Clone*). |
